...
<html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><meta name="robots" content="noindex"/><title>Page not found | ${htmlEscape(siteName)}</title><script>const base=${JSON.stringify(basePath)};const path=location.pathname;if(path.startsWith(base+"app/")){location.replace(base+"app/");}</script><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#06100e;color:#e8fff8;font-family:Inter,system-ui,sans-serif}.card{max-width:620px;padding:38px;border:1px solid rgba(141,255,235,.2);border-radius:28px;background:rgba(10,34,29,.72)}a{color:#8bfff4}</style></head><body><main class="card"><h1>Page not found</h1><p>The learning page may have moved. Open the simulator or return to the learning homepage.</p><p><a href="${publicPath("")}">Home</a> · <a href="${publicPath("app/")}">Open simulator</a></p></main></body></html>`;
